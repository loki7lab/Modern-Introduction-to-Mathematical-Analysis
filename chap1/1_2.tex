...
$a_0.a_1a_2\cdots a_n\cdots$是一个规范小数，它正好就是集合$E$的下确界。

假如$a_0.a_1a_2\cdots a_n$不是规范小数，
那么必定存在$p\in \mathbb{Z}^+$，使得
\[a_p+1 = a_p + 2 = \dots = 9\]
不妨设$p$是满足这个条件的最小的非负整数。
对任意的$\beta\in E$
，设$\beta$的规范小数表示为$\beta = \beta_0.\beta_1\beta_2\cdots$，
则必定存在$n>p$，使得$\beta_n < 9$。因为\[\beta \geq a_0.a_1\cdots a_n\]，
所以又必定存在$q \in \{0, 1, \dots, p\}$，使得

\begin{center}

$\beta_0 = a_0,\cdots ,\beta_{q-1} = a_{q-1},\beta_q \geq \alpha_q+1$
\end{center}
否则$\beta < a_0.a_1\cdots a_n$。于是有
\begin{align*}
...
我们看到，
\begin{center}
$\beta \geq a_0.a_1\cdots a_p, \forall \beta \in E$
\end{center}
由$a_0.a_1\cdots$是非规范小数的假定导出的这一结论，与$a_p$的选择办法相矛盾。
由此得知：$a_0.a_1\cdots a_n$必定是规范小数。\\

下面，我们来证明实数$a = a_0.a_1a_2\cdots$是集合$E$的下确界。
